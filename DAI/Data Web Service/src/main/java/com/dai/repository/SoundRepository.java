...
public interface SoundRepository
    ...
    @Query(nativeQuery = true, value = "SELECT  measuredDate, value, threshold, measurementId, type FROM (SELECT measured_date as measuredDate, sound as value, maximum as threshold, measurement_id as measurementId, type\n" +
    ...
    @Query(nativeQuery = true, value = "SELECT  measuredDate, value, threshold, measurementId, type FROM (SELECT measured_date as measuredDate, sound as value, minimum as threshold, measurement_id as measurementId, type\n" +
            "                FROM measurement JOIN threshold t on measurement.area_id = t.area_id AND t.type like :type WHERE measurement.area_id = :area_id AND\n" +
            "                convert(date ,measurement.measured_date) = :date) as measurements\n" +
            "                WHERE value < threshold ORDER BY measurementId DESC")
