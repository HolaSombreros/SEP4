...
public interface HumidityRepository extends JpaRepository<Measurement, Integer> {
    @Query(nativeQuery = true, value = "SELECT measured_date as measuredDate, humidity as value FROM measurement WHERE measurement.area_id = :area_id ORDER BY measurement_id DESC LIMIT 1")
