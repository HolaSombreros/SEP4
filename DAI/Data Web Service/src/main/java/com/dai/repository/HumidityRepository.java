...
public interface HumidityRepository extends JpaRepository<Measurement, Integer> {
    ...
    @Query(nativeQuery = true, value = "SELECT measured_date as measuredDate, humidity as value FROM measurement WHERE measurement.area_id = :area_id AND convert(date, measurement.measured_date) = :date ORDER BY measurement_id")
